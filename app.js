...
export default function SchoolPlatform() {
  // --- SYSTEM MODES ---
  const [systemMode, setSystemMode] = useState<'normal' | 'enrollment' | 'emergency'>('normal');
  ...
  const [enrollForm, setEnrollForm] = useState<EnrollmentData>({
    parentName: '',
    phone: '',
    childName: '',
    gradeSelection: '',
  ...
  const [enrollStatus, setEnrollStatus] = useState<{ status: string; txId?: string } | null>(null);

  // --- AI ASSISTANT STATE ---
  const [aiQuery, setAiQuery] = useState<string>('');
  const [aiResponse, setAiResponse] = useState<string>('Need help? Ask me about enrollment or updates.');
  ...
  const updateEnrollForm = (fields: Partial<EnrollmentData>) => {
  ...
  const submitEnrollment = async () => {
    ...
    try {
      const response = await new Promise<{ success: boolean; id: string }>((resolve) =>
      ...
      <div className="bg-gray-800 text-gray-300 text-[10px] px-2 py-1 flex justify-between items-center">
        ...
        <div className="flex gap-1">
          {['normal', 'enrollment', 'emergency'].map((m) => (
            <button
              key={m}
              onClick={() => setSystemMode(m as any)}
